      ...
      if( user ){
        ...
        authUser.ref.child('nickname').once('value', snapshot => {
          ...
          if (!authUser.nickname) {
            ...
            this.notifier.notify( `Welcome, ${authUser.nickname}.\n[autogenerated_name]\nYou dont\'t like it, do you? change in ${Text.c.trophy}`, 20000 );
          ...
          console.log(`Welcome, ${authUser.nickname}`)
    ...
    let userMenu =
      ...
      ? new Menu( this.canvas, {
        ...
        enter: (entryIndex,choice) => {
          ...
          if( choice == "SET NAME"){
            let editor = userMenu.exit( new TextEditor( this.canvas, N7e.user.nickname || '', newNickname => {
              if( !newNickname ){
                Sound.inst.effects.SOUND_ERROR.play( ODR.config.SOUND_SYSTEM_VOLUME/10 );
                return editor;
              } else {
